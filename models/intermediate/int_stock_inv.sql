...
, ticker_date as (
    select d.* , asset_type, ticker
    from dim_date d
    cross join (select asset_type, ticker, min(purchase_month) as min_date from transaction group by 1,2 ) x 
    where d.date_month >= min_date
    order by ticker, date_month
)
, fact_table as (
    select
        d.date_month, d.year_month, d.asset_type, d.ticker,  unit_owned,
